...
public class MetadataFileNameTable
        implements SystemTable
{
    ...
    public MetadataFileNameTable(SchemaTableName tableName, Table icebergTable)
    {
        this.icebergTable = requireNonNull(icebergTable, "icebergTable is null");
        tableMetadata = new ConnectorTableMetadata(requireNonNull(tableName, "tableName is null"),
                ImmutableList.<ColumnMetadata>builder()
                        .add(new ColumnMetadata(METADATA_FILE_NAME, VARCHAR))
                        .build());
    }
    ...
    private static List<Page> buildPages(ConnectorTableMetadata tableMetadata, Table icebergTable)
    {
        ...
        // Cast to BaseTable is safe because the icebergTable comes from IcebergUtil.getIcebergTableWithMetadata
        String location = ((BaseTable) icebergTable).operations().current().metadataFileLocation();
